...
def parse_cell(cell_name,cell_value,item):
    ...
    if field_type==FieldDescriptor.TYPE_INT32 or field_type==FieldDescriptor.TYPE_SINT32\
        ...
        log.write_log(log_str)
        return enum_value
    log.write_log("Error: parse_cell type:"+cell_name)
    return None

#解析excel单元格数组(以,分隔)
def parse_array_cell(cell_name,cell_value,item):
    log.write_log("parse_array_cell "+cell_name)
    values = []
...
def parse_field(cell_name,cell_value,item):    
    ...
    log.write_log("parse_field is_array: "+cell_name+" "+str(is_array))
    if is_array:
...
def get_item_array(excel_path,item_array):
    log.write_log("get_item_array "+excel_path)
